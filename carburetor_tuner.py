...
def normalize(number):
    """Normalize a number to be within the range [-2, 2]."""
    if(number > 2):
        return -2
    if(number < -2):
        return 2
...
def get_user_input(prompt, min_val=-2, max_val=2):
...
def find_solution(initial_array):
    """Find the most efficient steps to reach array[2, 2, 2] from the initial array."""
    visited = set()
    ...
    while queue:
        ...
        if all(num == 2 for num in current_array):
            return steps
...
print("Please describe your current carburetor graph values")
initial_array = []
for i in range(3):
...
if steps:
    for step in steps:
        ...
        parts = step.split()
        if parts[3] == 'sum':
            current_array[int(parts[3])] = normalize(current_array[int(parts[3])] + 1)
